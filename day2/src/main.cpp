...
int solution()
{
    return std::ranges::count_if(  //
        ...
        [](const auto& report)
        {
            return isReportSafe(report, 1, 3);
        });
}
}  // namespace part1
namespace part2
...
bool isReportSafe(const std::vector<int>& report, int minThreshold, int maxThreshold)
{
    ...
    auto validate = [sign, minThreshold, maxThreshold](const auto& report)
    {
        return findReportBreak(report, sign, minThreshold, maxThreshold);
    };
    ...
    auto index = std::distance(std::begin(report), it);
    if (index != 0)
        --index;
